...
`}async function Q(e){if(e.target===e.currentTarget)return;const s=e.target.dataset.filter;console.log(s);try{const t=await X(s);$.innerHTML=Z(t);const r=document.querySelector(".TitleExercises");r.innerHTML="Exercises",document.querySelector(".ExercisesForm").remove(),console.log(r)}catch(t){console.log(t)}}async function X(e=filterValueDefault){try{return(await u.get(`${w}/filters`,{params:{filter:e,page:1,limit:20}})).data.results}catch(s){console.log(s)}}function Z(e){return e.map(({name:t,filter:r,imgUrl:o})=>` <li class='ExercisesItem' data-filter='${r}' data-name='${t}'>
        <img class="ImgExercises" src="${o}" alt="${r}">
        <div>
          <p>${t}</p>
          <p>${r}</p>
        </div>
      </li>`).join("")}const D=document.querySelector(".SearchExercises"),ee=document.querySelector(".SearchInput"),te=document.querySelector(".SearchExercisesList");document.querySelector("#LoadMoreButton");let O="";D.addEventListener("submit",se);function se(e){e.preventDefault(),O=ee.value,re(O)}async function re(e){try{const s="https://energyflow.b.goit.study/api/exercises?",t=await u.get(s,{params:{bodypart:"back",keyword:e,page:1,limit:9}});oe(t.data.results)}catch(s){ie(s)}finally{D.reset()}}function oe(e){e.length===0?ae():e.forEach(s=>{const t=document.createElement("li");t.textContent=s.name,te.appendChild(t)})}function ie(e){console.log(e)}function ae(){F.error({title:"No Results",message:"Unfortunately, no results were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs"})}const m={form:document.querySelector(".footerForm"),input:document.querySelector(".formInput"),submit:document.querySelector(".modalButton")},ne="https://energyflow.b.goit.study/api/subscription",ce="feedback-form-state",k=JSON.parse(localStorage.getItem(ce));k!=null?m.input.value=k.email:m.input.value="";m.submit.addEventListener("click",le);async function le(e){const s=m.input.value.trim();e.preventDefault();try{const t=await u.post(ne,{email:s});F.show({position:"topRight",message:JSON.stringify(JSON.parse(t.request.responseText).message),maxWidth:"352",messageColor:"#fff",messageSize:"15px",backgroundColor:"rgba(27, 27, 27, 0.7)",close:!1,closeOnClick:!0}),m.input.value=""}catch(t){F.show({position:"topRight",message:JSON.stringify(JSON.parse(t.request.responseText).message),maxWidth:"352",messageColor:"#fff",messageSize:"15px",backgroundColor:"rgba(27, 27, 27, 0.7)",close:!1,closeOnClick:!0}),m.input.value=""}}const C="/Dyvani_programisty/assets/symbol-defs-2d4054da.svg",de=window.location.pathname;de.lastIndexOf("/");let b="favorites",g=localStorage.getItem(b);g||(localStorage.setItem(b,JSON.stringify([])),g="[]");let l=JSON.parse(g);const y=document.querySelector(".add-favorites"),c=document.querySelector(".favorites-list");let h="",x;const P=document.querySelector(".message-favorites"),p=document.querySelector(".favorites-pagination-block"),E=document.querySelector(".div-favorites-section"),N=document.querySelector(".favorites-list"),ue=document.querySelectorAll(".favorites-list-item");if(!g||l.length==0)P.classList.add("is-open-message-favorites"),p.classList.add("close");else if(g)try{l.forEach(e=>{const s=`<li class="favorites-list-item" id="${e.id}">
