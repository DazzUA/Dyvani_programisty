import{a as u,i as q}from"./assets/vendor-8cce9181.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))r(o);new MutationObserver(o=>{for(const i of o)if(i.type==="childList")for(const n of i.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&r(n)}).observe(document,{childList:!0,subtree:!0});function t(o){const i={};return o.integrity&&(i.integrity=o.integrity),o.referrerPolicy&&(i.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?i.credentials="include":o.crossOrigin==="anonymous"?i.credentials="omit":i.credentials="same-origin",i}function r(o){if(o.ep)return;o.ep=!0;const i=t(o);fetch(o.href,i)}})();const W=document.querySelector(".FilterButtons"),F=document.querySelector(".ExerciseFiltersList"),T=document.querySelector(".Pagination"),U="https://energyflow.b.goit.study/api";let H="Muscles",E=1,P=window.innerWidth,w=0;W.addEventListener("click",R);T.addEventListener("click",V);P<=375?w=8:(P<=768,w=12);async function B(){try{return(await u.get(`${U}/filters`,{params:{filter:H,page:E,limit:w}})).data}catch(e){console.log(e)}}async function J(){try{const e=await B().then(s=>{const{results:t,totalPages:r,page:o}=s;if(t&&t.length>0){I(t);const i=A(o,r);T.innerHTML=i}else console.error("No exercises found.")})}catch(e){console.error("Error fetching exercises:",e)}}J();async function R(e){e.preventDefault(),E=1;const t=e.target.dataset.filter;if(H=t,F.innerHTML="",Array.from(e.currentTarget.children).map(r=>{r.textContent!==e.target.textContent?r.classList.remove("ButtonIsActive"):e.target.classList.add("ButtonIsActive")}),e.target.tagName==="BUTTON")try{B(t).then(r=>{const{results:o,totalPages:i,page:n}=r;I(o);{const a=A(n,i);T.innerHTML=a}})}catch(r){console.log(r)}}async function V(e){E=e.target.textContent,Array.from(e.currentTarget.children).map(s=>{s.textContent!==E?s.classList.remove("PaginationBtnIsActive"):e.target.classList.add("PaginationBtnIsActive")}),F.innerHTML="";try{const{results:s,page:t,totalPages:r}=await B();if(t===r)return;I(s)}catch(s){console.log(s)}}function I(e){const s=e.map(({name:t,filter:r,imgUrl:o})=>` <li class="FilterList" data-filter>
        <img class="ImgExercises" src="${o}" alt="${r}">
        <div class="FilterText">
          <p class="FilterExercises">${t}</p>
          <p class="FilterName">${r}</p>
        </div>
      </li>`).join("");F.insertAdjacentHTML("beforeend",s)}function A(e,s){let t="";for(let r=1;r<=s;r+=1)t+=`<button class="PaginationBtn PaginationBtnIsActive" type="button">${r}</button>`;return t}document.querySelector(".filter-buttons");const k=document.querySelector(".exercise-filters-list"),j=document.querySelector(".ExercisesHead"),z=document.querySelector(".pagination"),L="https://energyflow.b.goit.study/api";k.addEventListener("click",_);async function _(e){if(e.target===e.currentTarget)return;k.classList.add("ExerciseCategoryList");const s=e.target.closest(".ExercisesItem");console.log(s);const t=s.dataset.filter,r=s.dataset.name;console.log(t),console.log(r);try{const o=await K(t,r);console.log(o),k.innerHTML=Y(o.results),j.innerHTML=G(r),document.querySelector(".ExercisesForm").addEventListener("submit",n(t,r));async function n(v){try{const f="https://energyflow.b.goit.study/api/exercises?",d=await u.get(f,{params:{bodypart:"back",keyword:v,page:1,limit:9}});renderExercises(d.data.results)}catch(f){handleError(f)}finally{searchForm.reset()}}const a=document.querySelector("#FilterBtn");console.log(a),a.addEventListener("click",Q),z.innerHTML=""}catch(o){console.log(o)}}async function K(e,s){try{return e==="Muscles"?(await u.get(`${L}/exercises`,{params:{muscles:s}})).data:e==="Body parts"?(await u.get(`${L}/exercises`,{params:{bodypart:s}})).data:(await u.get(`${L}/exercises`,{params:{equipment:s}})).data}catch(t){console.log(t)}}function Y(e){return e.map(({rating:t,name:r,burnedCalories:o,time:i,bodyPart:n,target:a})=>`<li class="WorkoutCard">
...
`}async function Q(e){if(e.target===e.currentTarget)return;const s=e.target.dataset.filter;console.log(s);try{const t=await X(s);k.innerHTML=Z(t);const r=document.querySelector(".title-exercises");r.innerHTML="Exercises",document.querySelector(".ExercisesForm").remove(),console.log(r)}catch(t){console.log(t)}}async function X(e=filterValueDefault){try{return(await u.get(`${L}/filters`,{params:{filter:e,page:1,limit:20}})).data.results}catch(s){console.log(s)}}function Z(e){return e.map(({name:t,filter:r,imgUrl:o})=>` <li class='ExercisesItem' data-filter='${r}' data-name='${t}'>
        <img class="img-exercises" src="${o}" alt="${r}">
        <div>
          <p>${t}</p>
          <p>${r}</p>
        </div>
      </li>`).join("")}const D=document.querySelector(".SearchExercises"),ee=document.querySelector(".SearchInput"),te=document.querySelector(".SearchExercisesList");document.querySelector("#LoadMoreButton");let O="";D.addEventListener("submit",se);function se(e){e.preventDefault(),O=ee.value,re(O)}async function re(e){try{const s="https://energyflow.b.goit.study/api/exercises?",t=await u.get(s,{params:{bodypart:"back",keyword:e,page:1,limit:9}});oe(t.data.results)}catch(s){ie(s)}finally{D.reset()}}function oe(e){e.length===0?ae():e.forEach(s=>{const t=document.createElement("li");t.textContent=s.name,te.appendChild(t)})}function ie(e){console.log(e)}function ae(){q.error({title:"No Results",message:"Unfortunately, no results were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs"})}const m={form:document.querySelector(".footerForm"),input:document.querySelector(".formInput"),submit:document.querySelector(".modalButton")},ne="https://energyflow.b.goit.study/api/subscription",ce="feedback-form-state",$=JSON.parse(localStorage.getItem(ce));$!=null?m.input.value=$.email:m.input.value="";m.submit.addEventListener("click",le);async function le(e){const s=m.input.value.trim();e.preventDefault();try{const t=await u.post(ne,{email:s});q.show({position:"topRight",message:JSON.stringify(JSON.parse(t.request.responseText).message),maxWidth:"352",messageColor:"#fff",messageSize:"15px",backgroundColor:"rgba(27, 27, 27, 0.7)",close:!1,closeOnClick:!0}),m.input.value=""}catch(t){q.show({position:"topRight",message:JSON.stringify(JSON.parse(t.request.responseText).message),maxWidth:"352",messageColor:"#fff",messageSize:"15px",backgroundColor:"rgba(27, 27, 27, 0.7)",close:!1,closeOnClick:!0}),m.input.value=""}}const C="/Dyvani_programisty/assets/symbol-defs-2d4054da.svg",de=window.location.pathname;de.lastIndexOf("/");let b="favorites",g=localStorage.getItem(b);g||(localStorage.setItem(b,JSON.stringify([])),g="[]");let l=JSON.parse(g);const y=document.querySelector(".add-favorites"),c=document.querySelector(".favorites-list");let h="",x;const M=document.querySelector(".message-favorites"),p=document.querySelector(".favorites-pagination-block"),S=document.querySelector(".div-favorites-section"),N=document.querySelector(".favorites-list"),ue=document.querySelectorAll(".favorites-list-item");if(!g||l.length==0)M.classList.add("is-open-message-favorites"),p.classList.add("close");else if(g)try{l.forEach(e=>{const s=`<li class="favorites-list-item" id="${e.id}">
